...
const moduleCode = fs.readFileSync(path.join(__dirname, '../src/vqdigitalwalletgoogle.js'), 'utf8');
...
// Execute the IIFE and capture VqDigitalWalletGoogle
let VqDigitalWalletGoogle;
try {
  ...
  const executeCode = new Function('window', 'global', 'document', 'console', 'setTimeout', 'clearTimeout', 'btoa', 'atob', 'crypto', 'Promise', 'Date', 'Math', 'Array', 'Object', 'JSON', 'Error', 'TypeError', `
    ${moduleCode}
    return window.VqDigitalWalletGoogle;
  `);
  
  VqDigitalWalletGoogle = executeCode(
    mockWindow,
    mockWindow,
    mockWindow.document,
    mockWindow.console,
  ...
  console.error('Failed to load VqDigitalWalletGoogle module:', error);
  throw error;
}
